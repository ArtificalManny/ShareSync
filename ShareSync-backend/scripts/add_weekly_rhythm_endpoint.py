...
def fail(message):
    print(f"\n[add_weekly_rhythm_endpoint] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)


def backup(path: Path):
    backup_path = path.with_name(f"{path.name}.bak-weekly-rhythm-{STAMP}")
    backup_path.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"[add_weekly_rhythm_endpoint] backup created: {backup_path}")
...
def patch_controller():
    ...
    if "weekly-rhythm" in source and "getWeeklyRhythm" in source:
        print("[add_weekly_rhythm_endpoint] controller already has weekly-rhythm route")
        return False
...
"""
    ...
    print(f"[add_weekly_rhythm_endpoint] patched controller: {CONTROLLER}")
    return True
...
def patch_service():
    ...
    if "async getWeeklyRhythm(userId: string): Promise<any>" in source:
        print("[add_weekly_rhythm_endpoint] service already has getWeeklyRhythm")
        return False
...
"""
    ...
    print(f"[add_weekly_rhythm_endpoint] patched service: {SERVICE}")
    return True


def main():
    print("[add_weekly_rhythm_endpoint] starting")
    ...
    print("")
    print("[add_weekly_rhythm_endpoint] done")
    print("")
    print("Next checks:")
    print("  npm run build")
    print('  rg -n "weekly-rhythm|getWeeklyRhythm|thisWeekTotal|momentumLabel|TASK_SHIPPED|source: \\'backend\\'" src/user -C 6')
    print("")
    print("Then restart your backend dev server and test:")
    print("  curl -i http://localhost:5001/api/users/me/weekly-rhythm")
    print("")
    print("Use the port/prefix your backend actually runs on, and include Authorization if needed.")
