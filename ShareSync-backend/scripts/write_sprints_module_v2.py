...
// - Do not initialize scheduling here.
// ═══════════════════════════════════════════════════════════════════════════════
...
def fail(message: str):
    print(f"\n[write_sprints_module_v2] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[write_sprints_module_v2] starting")
    ...
    if TARGET.exists():
        backup_path = TARGET.with_name(f"{TARGET.name}.bak-sprints-module-v2-{STAMP}")
        backup_path.write_text(TARGET.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"[write_sprints_module_v2] backup created: {backup_path}")

    TARGET.write_text(MODULE_FILE, encoding="utf-8")
    print(f"[write_sprints_module_v2] wrote: {TARGET}")
    ...
    print("")
    print("[write_sprints_module_v2] done")
    print("")
    print("Next checks:")
    print("  npm run build")
