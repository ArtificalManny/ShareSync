...
def fail(message: str):
    print(f"\n[write_sprints_module_v3] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[write_sprints_module_v3] starting")
    ...
    if TARGET.exists():
        backup_path = TARGET.with_name(f"{TARGET.name}.bak-sprints-module-v3-{STAMP}")
        backup_path.write_text(TARGET.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"[write_sprints_module_v3] backup created: {backup_path}")

    TARGET.write_text(MODULE_FILE, encoding="utf-8")
    print(f"[write_sprints_module_v3] wrote: {TARGET}")
    ...
    for marker in forbidden:
        if marker in written:
            fail(f"Safety check failed. Forbidden dependency text found: {marker}")

    print("")
    print("[write_sprints_module_v3] done")
    print("")
    print("Next checks:")
    print("  npm run build")
    print("  cat src/sprints/sprints.module.ts")
