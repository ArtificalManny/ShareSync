...
export class PresenceController {
  ...
  async updateOwnPresence(
    ...
    if (update.currentProject) {
      presence.currentProject = new Types.ObjectId(update.currentProject) as any;
    }
    if (update.currentPage) presence.currentPage = update.currentPage;
    if (update.currentPageTitle) presence.currentPageTitle = update.currentPageTitle;
    if (update.device) presence.device = update.device as any;
    if (update.privacySettings) presence.privacySettings = update.privacySettings as any;
    if (update.metadata) presence.metadata = update.metadata as any;
  ...
  async setStatus(
    ...
    if (body.status === UserStatus.ONLINE && typeof (presence as any).setOnline === 'function') {
      await (presence as any).setOnline();
    } else if (body.status === UserStatus.OFFLINE && typeof (presence as any).setOffline === 'function') {
      await (presence as any).setOffline();
    } else {
      await presence.save();
    }
  ...
  async updatePrivacy(
    ...
    presence.privacySettings = {
      ...(presence.privacySettings as any),
      ...(settings as any),
    } as any;
  ...
  async getOnlineUsers(): Promise<PresenceDocument[]> {
    // IMPORTANT: no `.lean()` (PresenceDocument[] return type)
  ...
  async updateActivity(
    ...
    if (typeof (presence as any).updateActivity === 'function') {
      await (presence as any).updateActivity(body.activityType);
      return presence;
    }
  ...
  async joinProject(
    ...
    presence.currentProject = new Types.ObjectId(projectId) as any;
  ...
  async leaveProject(@CurrentUser() user: any): Promise<PresenceDocument> {
    ...
    presence.currentProject = undefined as any;
