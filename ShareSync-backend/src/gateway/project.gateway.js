...
var ProjectGateway = function () {
    ...
    var _handleProjectUpdate_decorators;
    var ProjectGateway = _classThis = /** @class */ (function () {
        function ProjectGateway_1(projectsService) {
        ...
        ProjectGateway_1.prototype.handleJoinProject = function (projectId, client) {
            client.join(projectId);
            this.server.to(projectId).emit('userJoined', { userId: client.id, projectId: projectId });
        };
        ProjectGateway_1.prototype.handleLeaveProject = function (projectId, client) {
            client.leave(projectId);
            this.server.to(projectId).emit('userLeft', { userId: client.id, projectId: projectId });
        };
        ProjectGateway_1.prototype.handleProjectUpdate = function (data) {
            return __awaiter(this, void 0, void 0, function () {
                var projectId, user, action, timestamp, activity;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            projectId = data.projectId, user = data.user, action = data.action, timestamp = data.timestamp;
                            activity = "".concat(user.email, " ").concat(action, " at ").concat(timestamp);
                            return [4 /*yield*/, this.projectsService.addTeamActivity(projectId, activity)];
                        case 1:
                            _a.sent();
                            this.server.to(projectId).emit('projectUpdated', { user: user, action: action, timestamp: timestamp });
                            return [2 /*return*/];
                    }
                });
            });
        };
        return ProjectGateway_1;
    }());
    __setFunctionName(_classThis, "ProjectGateway");
    (function () {
        ...
        _handleProjectUpdate_decorators = [(0, websockets_1.SubscribeMessage)('projectUpdate')];
        ...
        __esDecorate(_classThis, null, _handleProjectUpdate_decorators, { kind: "method", name: "handleProjectUpdate", static: false, private: false, access: { has: function (obj) { return "handleProjectUpdate" in obj; }, get: function (obj) { return obj.handleProjectUpdate; } }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        ProjectGateway = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return ProjectGateway = _classThis;
}();
