...
export class AnalyticsService {
  ...
  async getUserProductivity(
    ...
    // 🛡️ CRITICAL FIX: Safe Native Query to prevent Dependency Injection Crash
    // This allows us to read the User document directly without risking analytics.module imports
    const userDoc = await this.eventLogModel.db.collection('users').findOne({ _id: new Types.ObjectId(userId) });
    const streakDays = userDoc?.streakDays || 0;

    // 📊 CALCULATE EFFICIENCY (Comparing current period to previous period)
    const periodDuration = endDate.getTime() - startDate.getTime();
    const prevStartDate = new Date(startDate.getTime() - periodDuration);
    
    const prevEvents = await this.eventLogModel.aggregate([
      {
        $match: {
          userId: new Types.ObjectId(userId),
    ...
    const prevCompletedCount = prevEvents[0]?.count || 0;
    const currentCompletedCount = tasksCompleted.count;
    let efficiency = 0;

    if (prevCompletedCount === 0) {
      efficiency = currentCompletedCount > 0 ? 100 : 0;
    } else {
      efficiency = Math.round(((currentCompletedCount - prevCompletedCount) / prevCompletedCount) * 100);
    }
    ...
    return {
      ...
      completionRate: tasksCreated.count > 0
        ...
        : 0,
      avgCompletionTime: 0, 
      pointsCompleted: tasksCompleted.value || 0,
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 📈 NEW: PROFILE ANALYTICS PIPELINE
  // ─────────────────────────────────────────────────────────────────────────────
  async getUserProfileAnalytics(userId: string): Promise<ProfileAnalyticsDto> {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    // 1. Quality Score
    const completions = await this.eventLogModel.find({
      userId: new Types.ObjectId(userId),
      type: EventType.TASK_COMPLETED,
      timestamp: { $gte: thirtyDaysAgo }
    });

    let qualityScore = 85; // Baseline
    if (completions.length > 0) {
       qualityScore = Math.min(99, 75 + (completions.length * 0.8));
    }

    // 2. Collaboration Score
    const collabs = await this.eventLogModel.aggregate([
      { $match: { userId: new Types.ObjectId(userId), projectId: { $exists: true }, timestamp: { $gte: thirtyDaysAgo } } },
      { $group: { _id: '$projectId', count: { $sum: 1 } } }
    ]);
    const collaborationScore = Math.min(98, 60 + (collabs.length * 12));

    // 3. Growth Trends
    const fourteenDaysAgo = new Date();
    fourteenDaysAgo.setDate(fourteenDaysAgo.getDate() - 14);
    const trends = await this.eventLogModel.aggregate([
      { $match: { userId: new Types.ObjectId(userId), type: EventType.TASK_COMPLETED, timestamp: { $gte: fourteenDaysAgo } } },
      { $group: {
    ...
    const growthTrends = trends.map(t => ({ date: t._id, value: t.value }));

    // 4. Trend Vector
    let trendVector = 'stable';
    if (growthTrends.length >= 2) {
       const recent = growthTrends.slice(-3).reduce((acc, val) => acc + val.value, 0);
       const older = growthTrends.slice(0, 3).reduce((acc, val) => acc + val.value, 0);
       if (recent > older) trendVector = 'up';
       else if (recent < older && recent !== 0) trendVector = 'down';
    }
