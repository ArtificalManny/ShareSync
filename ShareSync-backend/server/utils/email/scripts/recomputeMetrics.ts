// server/utils/email/scripts/recomputeMetrics.ts
// Run with ts-node or transpile. Schedules: daily + hourly.
...
async function main() {
  await mongoose.connect(MONGO_URL, DB_NAME ? { dbName: DB_NAME } as any : undefined);

  const Project = mongoose.model("Project", new mongoose.Schema(
    { title: String, public: Boolean, discoverable: Boolean, updatedAt: Date },
    { collection: "projects" }
  ));
  const Task = mongoose.model("Task", new mongoose.Schema(
    { projectId: mongoose.Schema.Types.ObjectId, status: String, completedAt: Date },
    { collection: "tasks" }
  ));
  const Activity = mongoose.model("Activity", new mongoose.Schema(
    { projectId: mongoose.Schema.Types.ObjectId, kind: String, createdAt: Date },
    { collection: "activities" }
  ));
  const XPEvent = mongoose.model("XPEvent", new mongoose.Schema(
    { projectId: mongoose.Schema.Types.ObjectId, amount: Number, createdAt: Date },
    { collection: "xp_events" }
  ));
  const Metrics = mongoose.model("Metrics", new mongoose.Schema(
    {
      projectId: mongoose.Schema.Types.ObjectId,
      // match the fields the route reads:
      throughputPerWeek: Number,
      xpDelta7d: Number,
      reactions7d: Number,
      updatedAt: Date,
      // optional: store 30/90d aggregates too:
  ...
  // Only public/discoverable projects
  ...
  const weeks = 1; // we used 7d window; adjust if you materialize more ranges
