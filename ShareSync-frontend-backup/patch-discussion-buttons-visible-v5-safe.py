...
backup_path = FILE_PATH.with_suffix(
    FILE_PATH.suffix + f".backup-discussion-buttons-visible-v5-safe-{timestamp}"
)
...
for line in lines:
    ...
    if stripped.startswith('className="') and 'discussion-primary-button' in line:
    ...
    if stripped.startswith('className="') and 'discussion-modal-create-button' in line:
        indent = line[: len(line) - len(line.lstrip())]
...
# Safer corruption checks only. Normal JSX arrow functions like onClick={() => ...} are allowed.
unsafe_patterns = [
    "onClick={() = className=",
    "onClick={()= className=",
    "onClick={className=",
...
print("Discussion button visibility v5 safe patch applied successfully.")
