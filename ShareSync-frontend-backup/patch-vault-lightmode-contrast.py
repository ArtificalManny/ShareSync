...
backup_path = FILE_PATH.with_suffix(
    FILE_PATH.suffix + f".backup-lightmode-contrast-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
)
backup_path.write_text(original)

updated = original

old_badge = """        <div className="absolute left-3 top-3 rounded-full border border-white/70 bg-white/85 px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-slate-600 shadow-sm backdrop-blur-xl dark:border-white/[0.08] dark:bg-black/35 dark:text-zinc-200">
          {fileType}
        </div>"""

new_badge = """        <div className="absolute left-3 top-3 rounded-full border border-slate-300/90 bg-slate-950/88 px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-white shadow-[0_10px_24px_rgba(15,23,42,0.24)] backdrop-blur-xl dark:border-white/30 dark:bg-black/80 dark:text-white">
          {fileType}
        </div>"""
...
replacements = [
    ("file type badge", old_badge, new_badge),
...
for label, old, new in replacements:
    ...
    if count != 1:
        raise RuntimeError(
            ...
            f"No changes written. Backup saved at {backup_path}"
        )
...
print("")
print("Changed only:")
print("- Upload button light-mode contrast")
print("- File type badge ('IMAGE', etc.) light-mode contrast")
print("")
