...
backup_path = FILE_PATH.with_suffix(
    FILE_PATH.suffix + f".backup-project-home-visual-strike-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
)
shutil.copy2(FILE_PATH, backup_path)

original = text

root_before = '    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-800 dark:text-zinc-100">'
root_after = '    <div className="project-home-visual-scope relative min-h-screen overflow-x-hidden bg-slate-50 dark:bg-[#0f172a] text-slate-800 dark:text-zinc-100">'

if root_before not in text and "project-home-visual-scope" not in text:
    raise RuntimeError("Could not find the ProjectHome root wrapper. No changes were written.")

if "project-home-visual-scope" not in text:
    text = text.replace(root_before, root_after, 1)

style_block = r'''      <style className="project-home-visual-style">{`
        .project-home-visual-scope {
          isolation: isolate;
          background:
            radial-gradient(circle at 16% 0%, rgba(139, 92, 246, 0.13), transparent 30%),
            radial-gradient(circle at 88% 10%, rgba(45, 212, 191, 0.14), transparent 32%),
            linear-gradient(180deg, #f8fafc 0%, #eef4f8 100%) !important;
        }

        .dark .project-home-visual-scope {
          background:
            radial-gradient(circle at 16% 0%, rgba(139, 92, 246, 0.20), transparent 32%),
            radial-gradient(circle at 86% 8%, rgba(34, 211, 238, 0.14), transparent 34%),
        ...
        .project-home-visual-scope > header button {
        ...
        .project-home-main-overview section h3,
        .project-home-main-overview section h4 {
        ...
        .project-home-main-overview article:hover {
          transform: translateY(-1px);
          border-color: rgba(139, 92, 246, 0.25) !important;
        }

        .project-home-main-overview button {
...
if "project-home-visual-style" not in text:
    ...
    if anchor not in text:
        raise RuntimeError("Could not find SHOW_DEBUG anchor for style insertion. No changes were written.")
    text = text.replace(anchor, style_block + "\n" + anchor, 1)

main_before = "      <main key={pulseRefreshKey}>{renderViewContent()}</main>"
main_after = '      <main key={pulseRefreshKey} className={`project-home-main project-home-main-${activeView}`}>{renderViewContent()}</main>'

if main_before in text:
    text = text.replace(main_before, main_after, 1)
elif "className={`project-home-main project-home-main-${activeView}`}" not in text:
    raise RuntimeError("Could not find main wrapper. No changes were written.")
...
print("ProjectHome visual strike patch applied successfully.")
