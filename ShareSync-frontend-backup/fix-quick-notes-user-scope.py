...
path = Path("src/context/NotesContext.jsx")

if not path.exists():
    raise FileNotFoundError("Could not find src/context/NotesContext.jsx")

stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
backup = path.with_suffix(path.suffix + f".backup-user-scoped-notes-{stamp}")
shutil.copy2(path, backup)

new_file = r'''// /src/context/NotesContext.jsx
import React, {
  createContext,
  useCallback,
  useContext,
...
print("Quick Notes user-scope fix applied successfully.")
print(f"Updated file: {path}")
print(f"Backup file:  {backup}")
print("")
print("Changed only:")
print("- Replaced global Quick Notes storage with per-user storage keys")
print("- Added account-switch detection")
print("- Added cross-tab sync for the current user's notes")
print("- Added window.__debugQuickNotesScope() for verification")
print("")
print("No backend files touched.")
print("No QuickNotesDrawer.jsx changes needed.")
print("No note UI styling changed.")
