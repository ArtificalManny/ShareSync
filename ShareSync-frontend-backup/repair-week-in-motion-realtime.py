...
backup = path.with_suffix(path.suffix + f".backup-realtime-week-in-motion-{stamp}")
shutil.copy2(path, backup)

content = r'''// src/components/home/WeekInMotion.jsx
// ═══════════════════════════════════════════════════════════════════════════════
// WeekInMotion — user-scoped weekly shipping rhythm
// Realtime-safe pass:
// - Uses /users/me/weekly-rhythm as preferred source
// - Falls back to /users/me/stats
// - Derives active days from real day counts when available
// - Refreshes on task/project/activity events, focus, visibility, and polling
...
function getDayIndexFromLabel(label) {
  ...
  rawDays.slice(0, 7).forEach((entry, fallbackIndex) => {
    ...
    if (entry && typeof entry === "object") {
      ...
      if (parsedDate) {
        ...
        const labelIndex = getDayIndexFromLabel(
          entry.day ?? entry.label ?? entry.weekday ?? entry.name
        );
...
function buildFallbackWeekDays({ weeklyShips = 0, activeDays = 0 } = {}) {
  ...
  for (
    let index = Math.max(0, todayIndex);
...
function getMomentum({ weeklyShips, activeDays }) {
  if (weeklyShips >= 8 && activeDays >= 3) return "strong";
  if (weeklyShips >= 3) return "rising";
  if (weeklyShips > 0) return "building";
  return "idle";
}
...
function getMomentumLabel(momentum) {
  ...
  return labels[momentum] || "Warming up";
}
...
function normalizeWeeklyRhythmPayload(rhythmPayload, statsPayload) {
  ...
  const rawWeeklyShips = readNumber(
    ...
    stats?.ships?.thisWeek
  );

  const rawActiveDays = readNumber(
    rhythm.activeDays,
  ...
  const lastWeekShips = readNumber(
  ...
  let days = normalizeIncomingDays(rawDays);
  let derivedWeeklyShips = days.reduce((sum, day) => sum + safeNumber(day.count, 0), 0);

  if (derivedWeeklyShips === 0 && rawWeeklyShips > 0) {
    days = buildFallbackWeekDays({
      weeklyShips: rawWeeklyShips,
      activeDays: rawActiveDays || 1,
    });

    derivedWeeklyShips = days.reduce((sum, day) => sum + safeNumber(day.count, 0), 0);
  }

  const weeklyShips = Math.max(rawWeeklyShips, derivedWeeklyShips);
  ...
  const momentum = rhythm.momentum || getMomentum({ weeklyShips, activeDays });
  const momentumLabel =
...
async function fetchWeeklyRhythm() {
  ...
  const [rhythmRes, statsRes] = await Promise.allSettled([
    client.get("/users/me/weekly-rhythm", requestConfig),
    client.get("/users/me/stats", requestConfig),
  ]);

  const rhythmPayload =
    rhythmRes.status === "fulfilled" ? unwrapApiPayload(rhythmRes.value) : null;

  const statsPayload =
    statsRes.status === "fulfilled" ? unwrapApiPayload(statsRes.value) : null;
...
export default function WeekInMotion({ className = "", onShipNow }) {
  ...
  const refetch = useCallback(async ({ showLoading = false } = {}) => {
    ...
    try {
      ...
      console.warn("[WeekInMotion] Fetch failed:", err?.message || err);
  ...
  useEffect(() => {
    ...
    const poll = setInterval(refreshSafely, 30000);
  ...
  const maxCount = useMemo(() => {
    ...
    return Math.max(...data.days.map((day) => safeNumber(day.count, 0)), 0);
  }, [data]);

  const peakDate = data?.peakDay?.date || data?.peakDate || null;
  const normalizedPeakDate = peakDate ? toLocalDateKey(parseLocalDate(peakDate)) : null;
  ...
  if (!data || !data.days?.length) {
    return (
      ...
      >
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-5 dark:border-white/10 dark:bg-white/[0.03]">
...
print("WeekInMotion realtime repair applied successfully.")
print(f"Updated file: {path}")
print(f"Backup file:  {backup}")
print("")
print("Changed only:")
print("- Replaced WeekInMotion.jsx with a realtime-safe version")
print("- Uses /users/me/weekly-rhythm first and /users/me/stats as fallback")
print("- Derives active days from actual day bars when available")
print("- Refreshes on task/project/activity events, browser focus, visibility, and 30s polling")
print("")
print("No backend files touched.")
print("No routing files touched.")
print("No unrelated Home.jsx layout changes.")
