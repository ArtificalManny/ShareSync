...
const ProjectHome = () => {
  ...
  if (authError || error) {
    return (
      <div className="project-home-container flex items-center justify-center min-h-screen">
        <p className="text-error-red text-lg font-orbitron">{authError || error}</p>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="project-home-container flex items-center justify-center min-h-screen">
        <p className="text-accent-teal text-lg font-orbitron">Project not found.</p>
      </div>
    );
  }
        ...
        {isEditingProject ? (
          <form onSubmit={handleProjectUpdate} className="max-w-2xl mx-auto">
            <input
              ...
              className="input-field text-4xl font-orbitron font-bold text-primary-blue mb-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-gray"
            ...
            <div className="flex gap-4 mt-4">
              <select
                ...
                className="input-field rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-gray"
          ...
          <div className="max-w-2xl mx-auto">
            <div className="flex justify-between items-center">
              <h1 className="text-4xl font-orbitron font-bold text-primary-blue mb-4">{project.title}</h1>
              <button
            ...
            <p className="text-accent-teal text-lg font-inter mb-4">{project.description}</p>
            <p className="text-secondary-gray font-inter">Category: {project.category}</p>
            <p className="text-secondary-gray font-inter">Status: {project.status}</p>
          </div>
        )}
      </div>

      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {error && <p className="text-error-red mb-4 text-center font-orbitron">{error}</p>}

        {/* Posts Section */}
        <div className="posts-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <FileText className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Posts
          </h2>
          <form onSubmit={handleCreatePost} className="mb-4">
            <select
              value={newPost.type}
              onChange={(e) => setNewPost({ ...newPost, type: e.target.value })}
              className="input-field rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-gray mb-2"
            ...
            <textarea
              ...
              className="input-field w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-gray mb-2"
          ...
          {(project.posts || []).length === 0 ? (
            <p className="text-accent-teal font-inter flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-neutral-gray animate-pulse" aria-hidden="true" /> No posts yet.
            </p>
          ) : (
            <div className="space-y-4">
              {project.posts.map((post, index) => (
                <div key={index} className="post-item p-4 bg-accent-teal bg-opacity-20 rounded-lg">
                  <p className="text-primary-blue font-orbitron font-medium">{post.type.toUpperCase()}</p>
                  <p className="text-secondary-gray font-inter">{post.content}</p>
        ...
        {/* Tasks Section */}
        <div className="tasks-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <CheckSquare className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Tasks
          </h2>
          ...
          {(project.tasks || []).length === 0 ? (
            <p className="text-accent-teal font-inter flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-neutral-gray animate-pulse" aria-hidden="true" /> No tasks yet.
            </p>
          ) : (
            <div className="space-y-4">
              {project.tasks.map((task) => (
                <div key={task._id} className="task-item p-4 bg-accent-teal bg-opacity-20 rounded-lg">
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-orbitron font-bold text-primary-blue">{task.title}</h3>
                    <select
                      value={task.status}
                  ...
                  <p className="text-secondary-gray font-inter">{task.description}</p>
                  <p className="text-secondary-gray text-sm font-inter">Assigned To: {task.assignedTo}</p>
                  <p className="text-secondary-gray text-sm font-inter">Due: {task.dueDate ? new Date(task.dueDate).toLocaleDateString() : 'N/A'}</p>
                  ...
                  <div className="subtasks mt-4">
                    <h4 className="text-md font-orbitron font-semibold text-primary-blue mb-2">Subtasks</h4>
                    {(task.subtasks || []).map((subtask) => (
                      <div key={subtask._id} className="subtask-item p-2 bg-neutral-gray bg-opacity-20 rounded-lg mb-2">
                        <p className="text-secondary-gray font-inter">{subtask.title}</p>
                      </div>
                    ))}
                    <form
                      onSubmit={(e) => {
                  ...
                  <div className="comments mt-4">
                    <h4 className="text-md font-orbitron font-semibold text-primary-blue mb-2">Comments</h4>
                    {(task.comments || []).map((comment, index) => (
                      <div key={index} className="comment-item p-2 bg-neutral-gray bg-opacity-20 rounded-lg mb-2">
                        <p className="text-secondary-gray font-inter">{comment.text}</p>
                        <p className="text-secondary-gray text-sm font-inter">By {comment.user} at {new Date(comment.timestamp).toLocaleString()}</p>
                      </div>
                    ))}
                    <form
                      onSubmit={(e) => {
        ...
        {/* Files Section */}
        <div className="files-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <FileText className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Files
          </h2>
          ...
          {(project.files || []).length === 0 ? (
            <p className="text-accent-teal font-inter flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-neutral-gray animate-pulse" aria-hidden="true" /> No files yet.
            </p>
          ) : (
            <div className="space-y-4">
              {project.files.map((file) => (
                <div key={file._id} className="file-item p-4 bg-accent-teal bg-opacity-20 rounded-lg flex justify-between items-center">
                  <div>
                    <p className="text-secondary-gray font-inter">{file.name}</p>
                    <p className="text-secondary-gray text-sm font-inter">Uploaded by {file.uploadedBy} at {new Date(file.uploadedAt).toLocaleString()}</p>
                    <p className="text-secondary-gray text-sm font-inter">Status: {file.status || 'Pending'}</p>
                  </div>
        ...
        {/* Teams Section */}
        <div className="teams-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <Users className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Teams
          </h2>
          ...
          {(project.teams || []).length === 0 ? (
            <p className="text-accent-teal font-inter flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-neutral-gray animate-pulse" aria-hidden="true" /> No teams yet.
            </p>
          ) : (
            <div className="space-y-4">
              {project.teams.map((team) => (
                <div key={team._id} className="team-item p-4 bg-accent-teal bg-opacity-20 rounded-lg">
                  <p className="text-primary-blue font-orbitron font-medium">{team.name}</p>
                  <p className="text-secondary-gray font-inter">Members: {team.members.join(', ')}</p>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Invite Users */}
        <div className="invite-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <Users className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Invite Users
          </h2>
        ...
        {/* Suggestions Section */}
        <div className="suggestions-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <MessageSquare className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Suggestions
          </h2>
          ...
          {(project.suggestions || []).length === 0 ? (
            <p className="text-accent-teal font-inter flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-neutral-gray animate-pulse" aria-hidden="true" /> No suggestions yet.
            </p>
          ) : (
            <div className="space-y-4">
              {project.suggestions.map((suggestion, index) => (
                <div key={index} className="suggestion-item p-4 bg-accent-teal bg-opacity-20 rounded-lg">
                  <p className="text-secondary-gray font-inter">{suggestion.content}</p>
                  <p className="text-secondary-gray text-sm font-inter">By {suggestion.user}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="notification-settings-section mb-8 card p-6 glassmorphic holographic-effect">
          <h2 className="text-2xl font-orbitron font-semibold text-primary-blue mb-4 flex items-center">
            <Settings className="w-5 h-5 mr-2 text-neutral-gray" aria-hidden="true" /> Notification Settings
          </h2>
          <form onSubmit={handleUpdateNotificationSettings}>
            <div className="flex items-center gap-2 mb-2">
              <input
                ...
                className="focus:ring-primary-blue"
                aria-label="Email Notifications"
              />
              <label htmlFor="emailNotifications" className="text-secondary-gray font-inter">Email Notifications</label>
            </div>
            <div className="flex items-center gap-2 mb-4">
              ...
              />
              <label htmlFor="inAppNotifications" className="text-secondary-gray font-inter">In-App Notifications</label>
            </div>
            <button
              type="submit"
              className="btn-primary rounded-full flex items-center focus:outline-none focus:ring-2 focus:ring-neutral-gray holographic-effect"
