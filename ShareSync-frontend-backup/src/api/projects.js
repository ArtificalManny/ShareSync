...
export async function listProjects(params = {}) {
  ...
  const { data } = await client.get(`/api/projects?${qs}`);
...
export async function shipProject(id) {
  const { data } = await client.post(`/api/projects/${id}/ship`);
  return data;
}

/** GET /api/projects/quick */
export async function getProjectsQuick(limit = 6) {
  const { data } = await client.get(`/api/projects/quick?limit=${Math.max(1, Math.min(12, Number(limit)))}`);
  return Array.isArray(data) ? data : [];
}