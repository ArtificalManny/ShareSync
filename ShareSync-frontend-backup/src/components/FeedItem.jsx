...
const FeedItem = ({
  ...
  const renderContent = () => {
    switch (item.type) {
      case 'activity':
        return (
          <p className="text-gray-700 dark:text-gray-300 font-sans text-sm">
            <span className="font-medium">{item.user}</span> {item.message} in{' '}
            <span className="text-purple-500">{item.projectTitle}</span>
          </p>
        );
      case 'announcement':
      case 'poll':
      ...
      case 'file':
        return (
          <p className="text-gray-700 dark:text-gray-300 font-sans text-sm">
            <span className="font-medium">{item.user}</span> shared a file: <a href={item.url} className="text-purple-500 hover:underline">{item.message}</a> in{' '}
            <span className="text-purple-500">{item.projectTitle}</span>
          </p>
        );
      default:
  ...
  return (
    <div className="feed-item bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl p-4 shadow-lg transition-transform duration-200 hover:scale-102 hover:shadow-xl">
      <div className="flex items-start gap-2 mb-2">
        ...
        <div className="flex-1">
          {renderContent()}
          <div className="text-gray-500 dark:text-gray-400 font-sans text-xs mt-1">
      ...
      <div className="flex items-center gap-4 mt-2">
        <button
          ...
          className="flex items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-purple-500 transition-colors duration-200"
        ...
        <button
          ...
          className="flex items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-teal-400 transition-colors duration-200"
        ...
        <button
          ...
          className="flex items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-pink-500 transition-colors duration-200"
      ...
      {expandedComments[index] && (
        <div className="comments mt-3">
          {(item.comments || []).map((comment, commentIndex) => (
            <div key={commentIndex} className="flex items-start gap-2 mb-2">
              <img
              ...
              />
              <div>
                <p className="text-gray-800 dark:text-gray-300 font-sans text-sm font-medium">{comment.username}</p>
          ...
          <form onSubmit={(e) => handleCommentSubmit(index, e)} className="mt-2 flex gap-2">
            ...
            <button
              type="submit"
              className="bg-purple-500 text-white p-1 rounded-full hover:bg-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-200 transform hover:scale-105"
