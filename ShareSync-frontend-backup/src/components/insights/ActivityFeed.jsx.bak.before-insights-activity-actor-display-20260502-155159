...
const ACTIVITY_CONFIG = {
  ...
  'file_deleted':    { icon: FileText,     label: 'deleted a file',        color: 'text-rose-600 dark:text-rose-400',     bg: 'bg-rose-100 dark:bg-rose-500/15',     dot: 'bg-rose-500' },
  ...
  'member_removed':  { icon: UserPlus,     label: 'removed a member',      color: 'text-rose-600 dark:text-rose-400',     bg: 'bg-rose-100 dark:bg-rose-500/15',     dot: 'bg-rose-500' },
  ...
  'debug.test':      { icon: Zap,          label: 'ran a test',            color: 'text-slate-500 dark:text-white/50',    bg: 'bg-slate-100 dark:bg-white/10',       dot: 'bg-slate-400' },
  // ✅ NEW: in_progress type for task fallback
...
async function fetchTaskBasedActivity(projectId, limit = 50) {
  if (!projectId) return [];
    ...
    for (const task of tasks) {
      ...
      if (task.status === 'done' && task.completedAt) {
        items.push({
          ...
          actorName: null, // We don't have actor name from task alone
        });
      }
  ...
  const loadActivities = useCallback(async () => {
    ...
    try {
      ...
      if (!items || items.length === 0) {
        items = await fetchTaskBasedActivity(projectId, limit);
      }
    ...
    <div className={`bg-white dark:bg-[#1f1f23] border border-slate-200 dark:border-white/[0.06] rounded-xl ${className}`}
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/[0.06]">
        ...
        <button onClick={loadActivities} disabled={loading}
          className="p-1.5 rounded-lg text-slate-400 dark:text-white/30 hover:bg-slate-100 dark:hover:bg-white/[0.06] disabled:opacity-50 transition-colors" title="Refresh">
      ...
      <div className="px-5 py-2 max-h-[600px] overflow-y-auto">
        ...
        {error && !loading ? (
          <div className="py-6 text-center">
            ...
            <button onClick={loadActivities} className="mt-2 text-xs text-violet-600 dark:text-violet-400 hover:underline">Try again</button>
          </div>
        ) : null}
        {!loading && !error && activities.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="text-xs text-slate-400 dark:text-white/30 mt-1">Create tasks, upload files, or ship updates to see activity here</p>
          </div>
        ) : null}
