...
export default function Navbar({ user, isDarkMode, toggleDarkMode }) {
  ...
  // upload -> POST /api/profile/upload-profile-picture  (adjust path if needed)
  const handleChange = async (e) => {
    ...
    try {
      ...
      // persist new user object
      localStorage.setItem('user', JSON.stringify(data.user));
      window.location.reload();          // simplest “refresh” – keeps your existing prop-shape
    } catch (err) {
      alert('Upload failed');
    } finally {
      setUploading(false);
