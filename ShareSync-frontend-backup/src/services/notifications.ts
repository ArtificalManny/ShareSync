/* Notifications service
   ...
   - Optimistic actions via useOptimisticMutation in components
*/
...
async function fetchNotifications(since?: number): Promise<InboxItem[]> {
  ...
  return arr.map(normalize).sort((a: InboxItem, b: InboxItem) => b.ts - a.ts);
}
...
class NotificationsClient {
  ...
  setIntervalMs(ms: number): void {
    this.intervalMs = Math.max(5_000, ms);
  }
  ...
  async snooze(id: string, minutes = 60): Promise<any> {
...
if (typeof window !== "undefined") {
  // @ts-ignore
  window.__SS_NOTIFICATIONS__ = defaultClient;
}