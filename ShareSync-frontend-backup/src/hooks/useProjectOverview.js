...
function buildFinishLine(
  ...
  const snapshotCompletedAt =
    ...
    null;
  const snapshotCompletedBy =
    completionSnapshot?.completedBy ||
    null;

  // IMPORTANT:
  // Finish Line should reflect whether the project is currently completed,
  // not merely whether a historical completion snapshot exists.
  const completedAt = currentCompletedAt;
  const completedBy = currentCompletedBy;
