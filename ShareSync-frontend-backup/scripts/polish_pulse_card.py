...
backup = path.with_suffix(f".jsx.bak-before-pulse-visual-polish-{datetime.now().strftime('%Y%m%d-%H%M%S')}")
backup.write_text(text)

# Icons needed for the polished Pulse card.
needed_icons = [
    "Activity",
    "Flame",
    "Zap",
...
if missing:
    ...
    print("ℹ️ Pulse lucide icons already imported.")

start = text.find("function OverviewPulseCard(")

if start == -1:
    raise SystemExit(
        "❌ Could not find function OverviewPulseCard.\n"
        "Run: rg -n \"function OverviewPulseCard|OverviewPulseCard\" src/pages/ProjectHome.jsx -C 12"
    )

brace_start = text.find("{", start)

if brace_start == -1:
    raise SystemExit("❌ Found OverviewPulseCard but could not find opening brace.")
...
if end is None:
    raise SystemExit("❌ Could not find end of OverviewPulseCard function.")

new_card = r'''function OverviewPulseCard({ pulse = {} }) {
  const today = Math.max(0, Number(pulse?.today) || 0);
  const inMotion = Math.max(0, Number(pulse?.inMotion) || 0);
  const blocked = Math.max(0, Number(pulse?.blocked) || 0);
  const ready = Math.max(0, Number(pulse?.ready) || 0);
...
print("")
print("✅ Pulse card visually polished.")
print("✅ Added live signal header, current readout, status badge, gradient rail, and stronger stat cards.")
print("✅ Backup created:", backup)
print("")
print("Inspect:")
print('rg -n "function OverviewPulseCard|Live Signals|Current Readout|signalCards|RadioTower" src/pages/ProjectHome.jsx -C 10')
