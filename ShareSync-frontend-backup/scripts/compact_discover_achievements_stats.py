...
# 1) Ensure React hooks + client import exist.
text = text.replace(
    "import React from 'react';",
    "import React, { useEffect, useMemo, useState } from 'react';"
)

if "../../api/client" not in text:
    # Add client import after lucide imports block.
    marker = "import { useIsMobile } from '../../hooks/useMediaQuery';"
    if marker in text:
        text = text.replace(marker, marker + "\nimport client from '../../api/client';")
    else:
        raise SystemExit("Could not find useIsMobile import anchor.")

# 2) Replace Achievements function signature + initial block.
old_sig = "const Achievements = ({ currentLevel = 1, currentXp = 0, currentStreak = 0, totalShips = 0 }) => {"
...
old_start = """  const isMobile = useIsMobile();

  const achievements = {"""

new_start = """  const isMobile = useIsMobile();
...
if old_start not in text:
    raise SystemExit("Could not find isMobile/achievements block.")

text = text.replace(old_start, new_start)

# 3) Replace milestone fields to use resolved values.
text = text.replace(
...
text = text.replace("unlocked: currentStreak >= 7", "unlocked: resolvedStreak >= 7")
text = text.replace("unlocked: totalShips >= 5", "unlocked: resolvedShips >= 5")
text = text.replace("unlocked: currentLevel >= 5", "unlocked: resolvedLevel >= 5")
text = text.replace("unlocked: currentStreak >= 100", "unlocked: resolvedStreak >= 100")
text = text.replace("unlocked: currentLevel >= 10", "unlocked: resolvedLevel >= 10")
text = text.replace("unlocked: currentLevel >= 20", "unlocked: resolvedLevel >= 20")

# 4) Close useMemo dependency array after achievements object.
old_close = """    ]
  };
"""

new_close = """    ]
  }), [resolvedLevel, resolvedXp, resolvedStreak, resolvedShips]);
"""

if old_close not in text:
    raise SystemExit("Could not find achievements object close.")

text = text.replace(old_close, new_close, 1)

# 5) Insert compact variant before mobile branch.
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="h-2 bg-slate-200 dark:bg-surface-2 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full transition-all duration-500" style={{ width: `${shipProgress}%` }} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="h-2 bg-slate-200 dark:bg-surface-2 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-orange-400 to-rose-500 rounded-full transition-all duration-500" style={{ width: `${streakProgress}%` }} />
            </div>
          </div>
        </div>
