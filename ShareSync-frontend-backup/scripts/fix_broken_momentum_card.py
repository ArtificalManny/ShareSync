...
backup = path.with_suffix(f".jsx.bak-before-fix-broken-momentum-card-{datetime.now().strftime('%Y%m%d-%H%M%S')}")
backup.write_text(text)

# Ensure icons exist in lucide import.
...
if missing:
    ...
    print("ℹ️ Momentum icons already imported.")
...
end = text.find("\nfunction PriorityStack", start)

if end == -1:
    raise SystemExit("❌ Could not find function PriorityStack after MomentumCard.")
...
text = text[:start] + new_card + "\n\n" + text[end:].lstrip()
path.write_text(text)

print("")
print("✅ Fixed broken MomentumCard syntax.")
print("✅ Removed leftover malformed `}) {` block.")
print("✅ Backup created:", backup)
print("")
print("Inspect:")
print('rg -n "function MomentumCard|function PriorityStack|\\}\\) \\{|Pace Signal|momentumGradient" src/pages/ProjectHome.jsx -C 8')
