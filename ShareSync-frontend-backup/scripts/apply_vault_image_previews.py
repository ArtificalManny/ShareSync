...
def fail(message):
    print(f"\n[apply_vault_image_previews] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[apply_vault_image_previews] starting VaultView image preview patch")
...
"""
    ...
    if "function getVaultFileUrl(file)" not in source:
      if helper_anchor not in source:
          fail(
              "Could not find the formatBytes helper anchor. "
              "No changes were written."
          )
      source = source.replace(helper_anchor, helper_insert, 1)
      print("[apply_vault_image_previews] added safe file URL/date helpers")
    else:
      print("[apply_vault_image_previews] file URL/date helpers already present")
...
"""
  ...
  const canPreviewImage = fileType === 'image' && Boolean(fileUrl) && !imageFailed;
...
"""
    ...
    if old_file_card in source:
        ...
        print("[apply_vault_image_previews] replaced FileCard with image-preview-aware version")
    elif "const canPreviewImage = fileType === 'image'" in source:
        print("[apply_vault_image_previews] FileCard already appears image-preview-aware")
    else:
        fail(
            "Could not find the exact FileCard block. "
            "This protects VaultView.jsx from a risky partial patch. "
            "Search manually for `function FileCard` and `file.fileUrl` before changing anything."
        )

    if source == original:
        print("[apply_vault_image_previews] VaultView.jsx already up to date")
        return

    backup = VAULT_VIEW.with_suffix(VAULT_VIEW.suffix + ".bak-vault-image-previews")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[apply_vault_image_previews] backup created: {backup}")

    VAULT_VIEW.write_text(source, encoding="utf-8")
    print(f"[apply_vault_image_previews] patched: {VAULT_VIEW}")

    print("")
    print("[apply_vault_image_previews] done")
    print("")
    print("Next checks:")
    print("  npm run build")
