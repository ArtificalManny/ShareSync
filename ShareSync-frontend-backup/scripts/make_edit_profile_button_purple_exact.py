...
def fail(message):
    print(f"\\n[make_edit_profile_button_purple_exact] ERROR: {message}\\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[make_edit_profile_button_purple_exact] starting")
    ...
    if OLD_BLOCK not in source:
        fail("Could not find the exact blue Edit Profile button block. No changes were written.")
    ...
    if "linear-gradient(135deg, #3B82F6 0%, #2563EB 100%)" in source:
        fail("Safety check failed: old blue gradient still exists after patch.")

    backup = PROFILE.with_suffix(PROFILE.suffix + f".bak-edit-profile-purple-{datetime.now().strftime('%Y%m%d-%H%M%S')}")
    backup.write_text(original, encoding="utf-8")
    print(f"[make_edit_profile_button_purple_exact] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[make_edit_profile_button_purple_exact] patched: {PROFILE}")

    print("")
    print("Next checks:")
    print("  npm run build")
    print("  rg -n \"Edit Profile|handleEditProfile|8B5CF6|7C3AED|6D28D9|shadow-violet|rounded-full\" src/pages/Profile.jsx -C 5")
