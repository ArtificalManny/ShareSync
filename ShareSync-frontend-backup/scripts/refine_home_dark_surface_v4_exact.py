...
BACKUP = ROOT / "src/pages/Home.jsx.bak.before-home-dark-surface-refine-v4-exact"


def fail(message: str) -> None:
    print(f"\n[refine_home_dark_surface_v4_exact] ERROR: {message}")
    sys.exit(1)
...
def main() -> None:
    print("[refine_home_dark_surface_v4_exact] starting")
  ...
  const sectionCardClasses = useMemo(() => {
    ...
    return `${base} ${shadow}`;
        ...
        """  // Section card classes with dark-mode surface refinement
  const sectionCardClasses = useMemo(() => {
    ...
    if not BACKUP.exists():
        ...
        print(f"[refine_home_dark_surface_v4_exact] backup created: {BACKUP}")
    else:
        print(f"[refine_home_dark_surface_v4_exact] backup already exists, preserved: {BACKUP}")
    ...
    print("\n[refine_home_dark_surface_v4_exact] complete")
    print("\nNext checks:")
    print("  npm run build")
