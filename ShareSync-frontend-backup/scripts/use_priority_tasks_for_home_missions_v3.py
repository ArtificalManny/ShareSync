...
def fail(message: str):
    print(f"\n[use_priority_tasks_for_home_missions_v3] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)


def backup(path: Path):
    backup_path = path.with_name(f"{path.name}.bak-priority-missions-v3-{STAMP}")
    backup_path.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"[use_priority_tasks_for_home_missions_v3] backup created: {backup_path}")
...
def main():
    print("[use_priority_tasks_for_home_missions_v3] starting")
    ...
    if "function toPriorityMissions(priorityTasks, projects)" not in source:
        ...
        print("[use_priority_tasks_for_home_missions_v3] inserted toPriorityMissions helper")
    else:
        print("[use_priority_tasks_for_home_missions_v3] helper already exists")
    ...
    if old_line in source:
        ...
        print("[use_priority_tasks_for_home_missions_v3] replaced missions derivation")
    elif "const priorityMissions = toPriorityMissions(priorityTasks, projects);" in source:
        print("[use_priority_tasks_for_home_missions_v3] missions derivation already patched")
    else:
    ...
    if source == original:
        print("[use_priority_tasks_for_home_missions_v3] no changes needed")
        return

    backup(TARGET)
    TARGET.write_text(source, encoding="utf-8")

    print(f"[use_priority_tasks_for_home_missions_v3] patched: {TARGET}")
    print("")
    print("[use_priority_tasks_for_home_missions_v3] done")
    print("")
    print("Next checks:")
    print("  npm run build")
