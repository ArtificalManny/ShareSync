...
def fail(message: str):
    print(f"\n[use_priority_tasks_for_home_missions] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)


def backup(path: Path):
    backup_path = path.with_name(f"{path.name}.bak-priority-missions-{STAMP}")
    backup_path.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"[use_priority_tasks_for_home_missions] backup created: {backup_path}")
 ...
 * - Therefore this function uses priority tasks only to rank/select projects.
 * - Each returned item keeps the real project id.
  ...
  for (const task of safeTasks) {
    ...
    const baseMission =
      projectMissionMap.get(projectId) ||
      {
        ...
        _id: projectId,
        projectId,
        title: task?.projectName || task?.project?.name || task?.project?.title || "Project",
        name: task?.projectName || task?.project?.name || task?.project?.title || "Project",
        status: "active",
        metrics: {},
        raw: {},
      };
...
def main():
    print("[use_priority_tasks_for_home_missions] starting")
    ...
    if "function toPriorityMissions(priorityTasks, projects)" not in source:
        ...
        if insert_after not in source:
            fail("Could not find insertion point after toMissions().")
        ...
        print("[use_priority_tasks_for_home_missions] inserted toPriorityMissions helper")
    else:
        print("[use_priority_tasks_for_home_missions] helper already exists")

    old_line = "  const missions = useMemo(() => toMissions(projects), [projects]);"
    new_line = """  const missions = useMemo(() => {
    ...
    if old_line in source:
        ...
        print("[use_priority_tasks_for_home_missions] replaced missions derivation")
    elif "const priorityMissions = toPriorityMissions(priorityTasks, projects);" in source:
        print("[use_priority_tasks_for_home_missions] missions derivation already patched")
    else:
    ...
    if source == original:
        print("[use_priority_tasks_for_home_missions] no changes needed")
        return

    backup(TARGET)
    TARGET.write_text(source, encoding="utf-8")

    print(f"[use_priority_tasks_for_home_missions] patched: {TARGET}")
    print("")
    print("[use_priority_tasks_for_home_missions] done")
    print("")
    print("Next checks:")
    print("  npm run build")
