...
def fail(message):
    print(f"\n[harden_profile_skill_profile_panel] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[harden_profile_skill_profile_panel] starting")
    ...
    if "buildSkillProfileSnapshot" in source and "skills={skillProfileSnapshot.skills}" in source:
        print("[harden_profile_skill_profile_panel] Skill Profile panel already appears hardened")
        return

    # 1) Add helpers after calculateReliability if not already present.
    if "const buildSkillProfileSnapshot" not in source:
        ...
        print("[harden_profile_skill_profile_panel] inserted Skill Profile helpers")
    ...
    if "const skillProfileSnapshot = useMemo(" not in source:
        ...
        print("[harden_profile_skill_profile_panel] inserted Skill Profile derived state")

    # 3) Replace only the Skill Profile panel block.
    ...
    if source == original:
        print("[harden_profile_skill_profile_panel] no changes needed")
        return

    backup = PROFILE.with_suffix(PROFILE.suffix + ".bak-skill-profile-panel")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[harden_profile_skill_profile_panel] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[harden_profile_skill_profile_panel] patched: {PROFILE}")
