...
def fail(message):
    print(f"\n[polish_profile_recent_ships_v2] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)
...
def replace_existing_recent_ships_section(source):
    if "<RecentShipsPanel ships={recentShips}" in source:
        return source

    main_grid_marker = "      {/* ═══════════════════════════════════════════════════════════════════\n          MAIN GRID"
    main_grid_idx = source.find(main_grid_marker)
    ...
    if header_idx == -1 or header_idx > main_grid_idx:
        return source[:main_grid_idx] + RECENT_SHIPS_CALL + source[main_grid_idx:]

    # Find a safe start before the old Recent Ships markup.
    search_start = max(0, header_idx - 5000)
    candidate_markers = [
        "\n      {/* Recent Ships",
    ...
    for marker in candidate_markers:
        idx = source.rfind(marker, search_start, header_idx)
        if idx != -1:
            starts.append(idx)

    if not starts:
        fail("Found Recent Ships but could not locate old section start. No changes were written.")

    start = max(starts)

    if start >= main_grid_idx:
        fail("Calculated Recent Ships section start is unsafe. No changes were written.")

    return source[:start] + "\n" + RECENT_SHIPS_CALL.rstrip() + "\n" + source[main_grid_idx:]

def main():
    print("[polish_profile_recent_ships_v2] starting")
    ...
    source = replace_existing_recent_ships_section(source)
    ...
    if source == original:
        print("[polish_profile_recent_ships_v2] no changes needed")
        return

    backup = PROFILE.with_suffix(PROFILE.suffix + ".bak-recent-ships-polish-v2")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[polish_profile_recent_ships_v2] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[polish_profile_recent_ships_v2] patched: {PROFILE}")
