...
ROOT = Path.cwd()
TARGET = ROOT / "src/components/members/MembersList.jsx"
BACKUP = ROOT / "src/components/members/MembersList.jsx.bak.before-role-menu"


def fail(message: str) -> None:
    print(f"\n[upgrade_members_list_role_menu] ERROR: {message}")
    sys.exit(1)


def main() -> None:
    print("[upgrade_members_list_role_menu] starting")
    ...
    if "RECOMMENDED_DISPLAY_ROLES" in source:
        fail("MembersList.jsx already appears to contain the role menu upgrade. Refusing to patch twice.")

    replacement = '''// src/components/members/MembersList.jsx - Display project members
...
'''
    ...
    if not BACKUP.exists():
        ...
        print(f"[upgrade_members_list_role_menu] backup created: {BACKUP}")
    else:
        print(f"[upgrade_members_list_role_menu] backup already exists, preserved: {BACKUP}")

    TARGET.write_text(replacement, encoding="utf-8")

    updated = TARGET.read_text(encoding="utf-8")
    required_markers = [
        "RECOMMENDED_DISPLAY_ROLES",
        "onUpdateMemberDisplayRole",
        "Assign display role",
        "This label does not change permissions.",
        "Custom role...",
    ...
    print("\n[upgrade_members_list_role_menu] complete")
    print("\nNext checks:")
    print("  npm run build")
    print("  rg -n \"RECOMMENDED_DISPLAY_ROLES|onUpdateMemberDisplayRole|Assign display role|Custom role|Remove from project|Click again to remove|member.displayRole|isModerator\" src/components/members/MembersList.jsx -C 6")
    print("  git diff -- src/components/members/MembersList.jsx")
