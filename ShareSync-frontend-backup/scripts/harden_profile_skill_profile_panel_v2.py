...
def fail(message):
    print(f"\n[harden_profile_skill_profile_panel_v2] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)

def main():
    print("[harden_profile_skill_profile_panel_v2] starting")
    ...
    if "buildSkillProfileSnapshot" in source and "skills={skillProfileSnapshot.skills}" in source:
        print("[harden_profile_skill_profile_panel_v2] Skill Profile panel already appears hardened")
        return

    if not re.search(r"skills=\{\s*skillProfile\.skills\s*\}", source, re.MULTILINE):
        fail("Could not find direct skillProfile.skills radar binding. No changes were written.")

    # 1) Add helpers after calculateReliability.
    if "const buildSkillProfileSnapshot" not in source:
        ...
        print("[harden_profile_skill_profile_panel_v2] inserted Skill Profile helpers")

    # 2) Add derived snapshot after useGrowthTrack destructuring.
    if "const skillProfileSnapshot = useMemo(" not in source:
        ...
        print("[harden_profile_skill_profile_panel_v2] inserted Skill Profile derived state")

    # 3) Replace the Skill Profile panel by anchors.
    ...
    if re.search(r"skills=\{\s*skillProfile\.skills\s*\}", source, re.MULTILINE):
        fail("Safety check failed: old direct skillProfile.skills radar binding still exists.")

    if source == original:
        print("[harden_profile_skill_profile_panel_v2] no changes needed")
        return

    backup = PROFILE.with_suffix(PROFILE.suffix + ".bak-skill-profile-panel-v2")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[harden_profile_skill_profile_panel_v2] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[harden_profile_skill_profile_panel_v2] patched: {PROFILE}")
