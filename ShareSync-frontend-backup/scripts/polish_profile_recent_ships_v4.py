...
def fail(message):
    print(f"\n[polish_profile_recent_ships_v4] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)
...
def insert_recent_ships_state(source):
    if re.search(r'const\s*\[\s*recentShips\s*,\s*setRecentShips\s*\]\s*=\s*useState', source):
        return source
    ...
    return source.replace(anchor, anchor + "\n  const [recentShips, setRecentShips] = useState([]);", 1)
...
def insert_recent_ships_fetch(source):
    ...
    if anchor not in source:
        fail("Could not find setMe(merged) anchor. No changes were written.")

    return source.replace(anchor, anchor + RECENT_SHIPS_FETCH, 1)

def find_section_start_before(source, text_index):
    ...
    search_from = max(0, len(lines) - 180)

    candidates = []
    for i in range(search_from, len(lines)):
        stripped = lines[i].lstrip()

        if stripped.startswith("{/*") and ("ship" in stripped.lower() or "recent" in stripped.lower()):
            candidates.append(i)

        if stripped.startswith("{isOwnProfile") or stripped.startswith("<section") or stripped.startswith("<div"):
            candidates.append(i)

    if not candidates:
        return -1

    line_index = candidates[-1]
    return sum(len(line) for line in lines[:line_index])

def replace_recent_ships_before_impact(source):
    if "<RecentShipsPanel ships={recentShips}" in source:
        return source

    recent_idx = source.find("Recent Ships")
    if recent_idx == -1:
        fail("Could not find Recent Ships heading. No changes were written.")

    impact_idx = source.find("{/* Impact Metrics */}", recent_idx)
    if impact_idx == -1:
        impact_idx = source.find("Impact Metrics", recent_idx)

    if impact_idx == -1:
        fail("Could not find Impact Metrics anchor after Recent Ships. No changes were written.")

    start = find_section_start_before(source, recent_idx)
    if start == -1 or start >= recent_idx:
        fail("Could not safely locate Recent Ships section start. No changes were written.")

    return source[:start] + "\n" + RECENT_SHIPS_CALL + source[impact_idx:]

def main():
    print("[polish_profile_recent_ships_v4] starting")
    ...
    source = replace_recent_ships_before_impact(source)

    safety_checks = [
        ("const RecentShipsPanel =", "RecentShipsPanel component"),
        ("const groupRecentShips =", "groupRecentShips helper"),
        ("const getShipTypeMeta =", "getShipTypeMeta helper"),
        ("<RecentShipsPanel ships={recentShips} loading={growthLoading} />", "RecentShipsPanel render call"),
        ("setRecentShips(", "recent ships setter"),
        ("Recent completed work and shipped deliverables", "panel subtitle"),
        ("No recent ships yet.", "empty state"),
        ("CheckCircle2", "CheckCircle2 icon"),
        ("Clock3", "Clock3 icon"),
        ("Rocket", "Rocket icon"),
        ("Flag", "Flag icon"),
        ("FileCheck2", "FileCheck2 icon"),
        ("Send", "Send icon"),
        ("ArrowUpRight", "ArrowUpRight icon"),
        ("{/* Impact Metrics */}", "Impact Metrics marker preserved"),
    ]

    for marker, label in safety_checks:
        if marker not in source:
            fail(f"Safety check failed. Missing {label}: {marker}")

    if not re.search(r'const\s*\[\s*recentShips\s*,\s*setRecentShips\s*\]\s*=\s*useState', source):
        fail("Safety check failed. recentShips useState declaration was not found after patch.")

    if source == original:
        print("[polish_profile_recent_ships_v4] no changes needed")
        return

    backup = PROFILE.with_suffix(PROFILE.suffix + ".bak-recent-ships-polish-v4")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[polish_profile_recent_ships_v4] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[polish_profile_recent_ships_v4] patched: {PROFILE}")

    print("")
    print("Next checks:")
    print("  npm run build")
    print("  rg -n \"RecentShipsPanel|groupRecentShips|getShipTypeMeta|recentShips|Recent completed work|No recent ships yet|CheckCircle2|Clock3|Impact Metrics\" src/pages/Profile.jsx")
