...
UPDATED = r'''// src/components/insights/ActivityFeed.jsx
// ═══════════════════════════════════════════════════════════════════════════════
...
function ActivityRow({ item, fallbackActor }) {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 dark:border-white/[0.04] last:border-b-0 group">
      ...
      <ActivityAvatar item={item} config={config} fallbackActor={fallbackActor} />
...
def main():
    print("[fix_insights_activity_feed_actor_display] starting")

    if not TARGET.exists():
        raise SystemExit(f"[fix_insights_activity_feed_actor_display] ERROR: missing {TARGET}")
    ...
    for marker in required_markers:
        if marker not in original:
            raise SystemExit(f"[fix_insights_activity_feed_actor_display] ERROR: expected marker not found: {marker}")

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    backup = TARGET.with_suffix(f".jsx.bak.before-insights-activity-actor-display-{timestamp}")
    ...
    for marker in verification_markers:
        if marker not in updated:
            raise SystemExit(f"[fix_insights_activity_feed_actor_display] ERROR: verification marker missing: {marker}")

    print(f"[fix_insights_activity_feed_actor_display] backup created: {backup}")
    print("[fix_insights_activity_feed_actor_display] complete")
    print()
    print("Next checks:")
    print("  npm run build")
    print('  rg -n "ActivityAvatar|fetchCurrentUserSnapshot|buildNormalizedActivityItem|fallbackActor|actorAvatar|Project member|Team member" src/components/insights/ActivityFeed.jsx -C 6')
