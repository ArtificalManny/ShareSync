...
def fail(message):
    print(f"\n[polish_profile_recent_ships_v3] ERROR: {message}\n", file=sys.stderr)
    sys.exit(1)
...
def find_section_start_before(source, text_index):
    """
    Finds the safest JSX section start before the visible Recent Ships heading.
    We search upward by line, not by exact old class names.
    """
    ...
    # Search a generous range above the heading.
    search_from = max(0, len(lines) - 160)

    candidates = []
    for i in range(search_from, len(lines)):
        stripped = lines[i].lstrip()

        if stripped.startswith("{/*") and ("Recent Ships" in stripped or "recent" in stripped.lower() or "ships" in stripped.lower()):
            candidates.append(i)
...
def replace_recent_ships_before_impact(source):
    ...
    return source[:start] + replacement + source[impact_idx:]

def main():
    print("[polish_profile_recent_ships_v3] starting")
    ...
    if source == original:
        print("[polish_profile_recent_ships_v3] no changes needed")
        return

    backup = PROFILE.with_suffix(PROFILE.suffix + ".bak-recent-ships-polish-v3")
    if not backup.exists():
        backup.write_text(original, encoding="utf-8")
        print(f"[polish_profile_recent_ships_v3] backup created: {backup}")

    PROFILE.write_text(source, encoding="utf-8")
    print(f"[polish_profile_recent_ships_v3] patched: {PROFILE}")
