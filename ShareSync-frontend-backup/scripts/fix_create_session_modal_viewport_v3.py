...
def fail(msg):
    print(f"\n[fix_create_session_modal_viewport_v3] ERROR: {msg}\n", file=sys.stderr)
    sys.exit(1)


def replace_any(source, candidates, replacement, label):
    for candidate in candidates:
        if candidate in source:
            print(f"[fix_create_session_modal_viewport_v3] replaced: {label}")
...
def main():
    print("[fix_create_session_modal_viewport_v3] starting")
    ...
    # 1) Overlay: place modal lower than navbar, but allow vertical scrolling.
    source = replace_any(
        source,
        [
        ...
        'className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/45 px-4 pt-10 pb-6 backdrop-blur-md sm:px-5 sm:pt-12 sm:pb-8"',
        "overlay viewport spacing",
    )

    # 2) Panel: hard cap height to visible browser area and make it narrower.
    source = replace_any(
        source,
        [
        ...
        "modal panel hard viewport cap",
    )

    # 3) Inner z wrapper: make it a real flex column, so form can scroll.
    source = replace_any(
        source,
        [
    ...
    # 4) Header: shrink only; slightly compact.
    source = replace_any(
        source,
        [
        ...
        'className="shrink-0 flex items-start justify-between gap-3 border-b border-slate-200/70 px-5 py-3 dark:border-white/[0.06]"',
        "compact fixed header",
    )

    # 5) Header description: reduce wrap height slightly.
    source = source.replace(
        'className="mt-1 text-sm text-slate-500 dark:text-white/45"',
        'className="mt-1 max-w-sm text-xs leading-relaxed text-slate-500 dark:text-white/45"',
        1,
    )

    # 6) Form: this is the key. It must be min-h-0 and overflow-y-auto.
    source = replace_any(
        source,
        [
        ...
        'className="min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-4 overscroll-contain"',
        "scrollable form body",
    )

    # 7) Tighten section labels and vertical spacing by reducing large section gaps.
    source = source.replace("space-y-3.5", "space-y-3")
    source = source.replace("space-y-4", "space-y-3")

    # 8) Make title input slightly shorter.
    source = source.replace(
    ...
    # 9) Make the shared input class more compact.
    source = source.replace(
        'px-4 py-3 text-sm',
        'px-4 py-2.5 text-sm',
    )

    # 10) Make textarea shorter.
    source = source.replace(
    ...
    # 11) Footer: always visible at bottom of modal form.
    source = replace_any(
        source,
        [
        ...
        'className="sticky bottom-0 -mx-5 flex shrink-0 flex-col-reverse gap-3 border-t border-slate-200/70 bg-white/95 px-5 pt-3 pb-2 backdrop-blur-md dark:border-white/[0.06] dark:bg-[#101827]/95 sm:flex-row sm:justify-end"',
        "sticky visible footer",
    )

    # 12) Ensure primary action label is Add Session.
    ...
    backup_path = TARGET.with_name(f"{TARGET.name}.bak-fix-viewport-v3-{STAMP}")
    backup_path.write_text(original, encoding="utf-8")
    TARGET.write_text(source, encoding="utf-8")

    print(f"[fix_create_session_modal_viewport_v3] backup created: {backup_path}")
    print(f"[fix_create_session_modal_viewport_v3] patched: {TARGET}")
    print("")
    print("Next checks:")
    print("  npm run build")
    print('  rg -n "max-h-\\[calc\\(100dvh-7rem\\)|max-w-md|min-h-0 flex-1|overflow-y-auto|overscroll-contain|sticky bottom-0|Add Session|Cancel" src/calendar/CreateSessionModal.jsx -C 4')
