...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      // Fetch user settings (e.g., email notification preference)
      // For now, we'll assume a default value
  ...
  const handleUpdatePreferences = async (e: React.FormEvent) => {
    ...
    setSuccessMessage('');
    try {
      // Update user preferences (e.g., email notifications)
      // For now, we'll just log the action
      console.log('Updating email notifications preference:', emailNotifications);
      setSuccessMessage('Preferences updated successfully');
    } catch (error: any) {
      console.error('Update preferences error:', error.response?.data || error.message);
      setErrorMessage(error.response?.data?.error || 'An error occurred while updating preferences');
    }
  };
